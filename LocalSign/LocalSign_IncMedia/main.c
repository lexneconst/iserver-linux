...
int main(int argc, char **argv)
    ...
    BIO       *certs = BIO_new_file("certs/caseniumsign.crt", "w");
    BIO       *lkeys = BIO_new_file("certs/caseniumsign.key", "w");
    
    BIO       *keys  = BIO_new_file("certs/localsign.key", "r");
    BIO       *imps  = BIO_new_file("certs/localsign.crt", "r");
	...
	mkcert(&px5s, &x509, &pkey, &nkey ,2048,0xEF59EAFA,365*4);
    if(x509 ==NULL){
        printf("error: x509 \n");
        return 0;
    }
...
int mkcert(X509 **x509x,X509 **x509p, EVP_PKEY **pkeyp,
	...
	*nkeyp= pk;
	...
	X509_set_pubkey(x,pk);
  	...
  	name=X509_get_issuer_name(*x509x);
	...
	X509_NAME_add_entry_by_txt(name,"OU", 
	  MBSTRING_ASC, (unsigned char *)"LocalSign Class 2 Public Certification Authority", -1, -1, 0);

    X509_NAME_add_entry_by_txt(name,"CN",
				MBSTRING_ASC, 
		(unsigned char *)"LocalSign Extended Validation SHA256 SSL CA", -1, -1, 0);

	X509_set_subject_name(x,name);
    
    add_ext(x, NID_info_access , "caIssuers;URI:http://www.casenium.com/ca2048.crl");
    
	ex = new_cert_policies(SN_any_policy);
    if(ex){
      X509_add_ext(x, ex, -1);
    ...
    add_ext(x, NID_crl_distribution_points,"URI:http://www.casenium.com/localsign.crl");
	
        
    ex =new_subject_alt("PicoSignMGC-8-183");
	if(ex == NULL){
    ...
    //add_ext(x, NID_netscape_cert_type ,"sslCA,emailCA");
    
    
    
                                 //"OCSP;URI:http://www.lexne.com:8090/");
	//add_ext(x, NID_ext_key_usage,"msSGC,nsSGC");//"msSGC,nsSGC");
    ...
    */
    ...
    #ifdef __STD__
    X509_alias_set1(x, (unsigned char *)"Casenium", 15); 
...
int new_x509_crl(X509_CRL **crl,X509 *x509, EVP_PKEY *pkey)
{
    ...
    if(crlX){
      ...
      for (i=0; i<X509_get_ext_count(x509); i++){
	    ...
	    printf(">[%s] \n", OBJ_nid2sn( X509_EXTENSION_get_object(OBJ_obj2nid(ext) )));
	    printf("?[%s] \n", OBJ_nid2ln( X509_EXTENSION_get_object(OBJ_obj2nid(ext) )));
//	rintf(":\n"/);
...
do{  
    if(imp==NULL){
        printf("[%s] (%s=%d) ASN1_OCTET_STRING_set failed.,", __FILE__, __FUNCTION__, __LINE__);
        break;
    }
...
X509_EXTENSION *new_cert_policies(char *stringc)
{
    ...
    strcpy(url, "http://www.casenium.com/cps");
    ...
    qual->d.usernotice = note;
		...
		strcpy(buffer, " PicoSign Trusted Network .\r\n");
		//lstrcat(buffer, "\r\n");      
        //lstrcat(buffer," Copyright (c) 2012 LocalSign - Lexne, Inc.");
...
X509_EXTENSION *new_usage_period(void)
{
	...
	if(usage->notAfter == NULL || !usage->notBefore){
		usage->notAfter = ASN1_GENERALIZEDTIME_new(); //ASN1_GENERALIZEDTIME_set(NULL,time(NULL)); //M_ASN1_GENERALIZEDTIME_new();
		usage->notBefore = ASN1_GENERALIZEDTIME_new(); //ASN1_GENERALIZEDTIME_set(NULL,time(NULL)+2); //M_ASN1_GENERALIZEDTIME_new();
	}
