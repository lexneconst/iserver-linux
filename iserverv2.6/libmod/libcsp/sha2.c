...
/*** SHA-256: *********************************************************/
void
CSHA256_Init(CSHA256_CTX *context)
{
	if (context == NULL)
		return;
...
void
CSHA256_Transform(u_int32_t state[8], const u_int8_t data[SHA256_BLOCK_LENGTH])
{
...
void
CSHA256_Update(CSHA256_CTX *context, const u_int8_t *data, size_t len)
{
	...
	if (usedspace > 0) {
		...
		if (len >= freespace) {
			...
			len -= freespace;
			data += freespace;
			CSHA256_Transform(context->state, context->buffer);
		} else {
	...
	while (len >= SHA256_BLOCK_LENGTH) {
		...
		CSHA256_Transform(context->state, data);
...
void
CSHA256_Pad(CSHA256_CTX *context)
{
...
void
CSHA256_Final(u_int8_t digest[SHA256_DIGEST_LENGTH], CSHA256_CTX *context)
{
	CSHA256_Pad(context);
...
/*** SHA-512: *********************************************************/
void
CSHA512_Init(CSHA512_CTX *context)
{
	if (context == NULL)
		return;
...
void
CSHA512_Transform(u_int64_t state[8], const u_int8_t data[SHA512_BLOCK_LENGTH])
{
...
void
CSHA512_Update(CSHA512_CTX *context, const u_int8_t *data, size_t len)
{
	size_t	freespace, usedspace;
	...
	if (usedspace > 0) {
		...
		if (len >= freespace) {
			...
			len -= freespace;
			data += freespace;
			CSHA512_Transform(context->state, context->buffer);
		} else {
			/* The buffer is not yet full */
			memcpy(&context->buffer[usedspace], data, len);
	...
	while (len >= SHA512_BLOCK_LENGTH) {
		/* Process as many complete blocks as we can */
		CSHA512_Transform(context->state, data);
...
void
CSHA512_Pad(CSHA512_CTX *context)
{
	unsigned int	usedspace;
...
void
CSHA512_Final(u_int8_t digest[SHA512_DIGEST_LENGTH], CSHA512_CTX *context)
{
	CSHA512_Pad(context);
